...
```{r}
prediccion_data <- ifelse(predictions > 0.5, 1, 0)
# We create a confusion matrix with the obtained predictions
table(test_dat$Attrition, prediccion_data)
```
...
Nevertheless, we will test the model's predictions with other cut-off values. Since this model is very good at predicting the negative cases, but not the positive ones, thereby lowering the requirement for considering an observation as a positive case.

**We try a cut-off point of 0.25**


```{r}
prediccion_data <- ifelse(predictions > 0.25, 1, 0)
# We create a confusion matrix with the obtained predictions
table(test_dat$Attrition, prediccion_data)
```

We can see that the number of cases predicted as positive has increased, and the number of cases predicted as negative has decreased. However, the number of correct cases predicted as 0 has not decreased a lot, but the number of cases correctly classified as 1, has practically duplicated. Nevertheless, the number of false positives, has increased in a higher proportion.

Even so, depending on the use given to the model, this may not be something negative. If is more important to maximize the number of positive cases detected than minimizing the number of incorrectly cases classified as positive, this model is usefull.

**We try a cut-off point of 0.2**

```{r}
# Probamos con 0.20
prediccion_data <- ifelse(predictions > 0.20, 1, 0)
# We create a confusion matrix with the obtained predictions
table(test_dat$Attrition,prediccion_data)
```

In this case, the increment of true positives is insignificant (only 3 more cases were detected), compared with the decrease of true negatives detected (27 less cases were correctly detected). We can not say that this model is better than the previous one.

## 8.4 - Evaluating the selected model

We will calculate the metrics of the second confusion matrix that we obtained (that one that had the cut-off point of 0.25).
...
We can see that the metrics of this model are better than the obtained with the last decision tree. The single metric that has decreased, is specificity (has decreased from 0.91 to 0.86). All the rest metrics are higher, and some of them have significant increases, like for example, the sensitivity (has increased from 0.33 to 0.67), and the F-Measure (has increased from 0.36 to 0.57). We can attribute this increase to the fact that the number of true negatives has not decreased hugely, but the number of true positives has duplicated. This means that the relation between the number of true positives and true negatives is better in this model.
