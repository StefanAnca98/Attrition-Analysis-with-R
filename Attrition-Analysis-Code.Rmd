...
After attempting to use some relatively complex models, and getting not very satisfactory results (the decision tree model is not effective at identifying the positive class, the hierarchical clustering has a very low average silhouette even in the best obtained result, and the DBSCAN algorithm finds only one cluster, which makes unclear if this method is much better than just using a multivariate outlier detection method). 

Therefore, we will use a simpler model. We will use the logistic regression. This will serve as a benchmark. If other models are trained, they should at least work better than this one to be considered an improvement.

**Note.**

The best way to train this model is to verify all the statistical assumptions of the model, and only use the model if they are met. However, since we are using cross validation to evaluate its performance, we will accept not verifying them if the model performs well with other data than the training data. However, in a real scenario, where critical problems are analysed with the model, it is not a good practice to use the model without meeting the statistical assumptions as this could lead to non-generalizable results.
...
This ROC curve is moved away from the diagonal, to the upper left corner, which is something positive because it means that while the cut-off point increases, the false positives increment is slower than the true positives increment. For example, in this graph we can see a zone where the rate of false positives is still quite low (around 0.4), but the true positive rate is much more higher (around 0.8).
...
This model works better for predicting class 0 than the last obtained decision tree, predicting correctly 240 cases compared with the 226 predicted by the previous model. On the other hand, it predicts almost the same number of class 1 cases, 14 cases compared with the 15 predicted by the last obtained decision tree. At the same time, the number of false positives has decreased (there were 22 in the previous model, and in this one, there are 8). Therefore, we obtained a slightly better model in this case.

Nevertheless, we will test the predictions of the model with other cut-off point values. Since this model is very good at predicting the negative class but not the positive, we will decrease the cut-off point value, thereby lowering the requirement for considering an observation as a positive case.
