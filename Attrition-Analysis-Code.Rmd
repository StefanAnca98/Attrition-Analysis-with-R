...
If the project is visualized on a mobile device, the text in the code blocks may extend beyond the edges. When viewing this project on a mobile device, it is recommended to hold it in a **horizontal position**, as less text will extend beyond the edges in this orientation.

## 1.2 - Goal of the project.

The objective of this project will be to describe and explain how job abandonment occurs. Associations between different variables and job abandonment will be sought to describe when this occurs and to establish possible explanations for the phenomenon. Multiple techniques will be used for variable manipulation, data cleaning, data exploration, and clustering models to try to identify specific groups. The main goal is to explore the functioning of the algorithms and demonstrate ways of working with data at the time that we attempt a specific goal, rather than creating the best model.

Some steps in this project are designed to showcase a broad range of data science techniques, even if not all are directly interconnected or essential for the final outcome. Certain analyses were conducted to enhance understanding of the problem, and some steps were taken to prepare the data for potential future scenarios. This approach was intentionally chosen to demonstrate a variety of data science methods and skills.
...
Only observing the number of unique values we can observe some interesting things. First, "Over18" has only 1 unique value. In addition, there are multiple variables with only 2 or 3 unique values, and just 2 variables have more than 3 unique values. We will explore this variables starting from those that have less unique values to those that have more.

```{r}
# We show the unique value of the variable "Over18"
unique(HR_data$Over18)
```

The unique value is "Y". This indicates that all individuals are adults. In the data cleaning process, we will discard this variable, as is a constant that we already know from this point about the dataset.
...
First, we preparate the data for it.
...
data_long <- data_2_3_unique %>% 
  ...
  #We count how many times appears each category of each variable  
...
We can observe several things. First, for binary variables, in all cases, one of the variables is more dominant than the other. On the other hand, regarding the variables with 3 categories, first, in the 'Department' variable, there are only 3 unique values. This indicates that the sample is not representative of all types of jobs that might exist, but rather that it likely comes from a specific sector. Additionally, the 'Human Resources' category is underrepresented (only 4.3% of the cases).Regarding 'BusinessTravel,' we see a clear predominance of people who rarely travel over those who travel frequently or do not travel at all. Finally, concerning the 'MaritalStatus' variable, there is some balance between the proportions of the 3 categories, with only a slight predominance of the 'Married' category.
...
For both variables, the categories are limited to a really specific field. It seems as though this data is related to the healthcare sector, and includes some complementary departments like Marketing or Human Resources. The number of employees in each category varies significantly across all categories.

## 2.2 - Cathegoric variables stored as numeric.

Now we will start analyzing the numeric variables. First we will analyze some variables that were marked as categorical in the previously created variable dictionary, but which we did not find among the character class variables, specifically:
...
In all these cases, these are variables whose values are encoded as integers, but on the download page, they are recorded as categorical. Therefore, the numbers probably represent categories whose values we cannot necessarily be treated as continuous. Given the names, descriptions and data type of the variables, we should probably interpret these values as ordinal. In other words, with these variables, we can determine in which observation there are higher levels of a certain variable, but we cannot make arithmetic comparisons between the different levels (for example, we cannot say that 2 represents double of 1 in a specific variable).
...
We find very limited ranges for these variables. For all the values of the variables, there are bars with a minimum number of observations so that they are identifiable. The only issue we could point out is that since there are no labels showing the meaning of each value, we cannot distinguish whether the maximum and minimum values are the theoretical of the variable, or just the maximums/minimums within this sample. To address this issue, when we will prepare the data, we will create some variables from these to allow us to establish clear differences between the subjects if we use these variables.
...
There are several things we can observe in this summary.
...
We can see that their values are discrete since each one has the same number of unique values as bars in the histogram. They seem to be values from a questionnaire. Therefore, we encounter the same issue as before: we don't know if the minimum and maximum observed in this sample are the true minimum and maximum values that these variables can take, or if they could actually have higher or lower values. Additionally, we do not know the exact meaning of each level of the categories of these variables.
...
We can see multiple things. First, some of the boxplots are very similar to a normal distribution, as the median is right in the center of the figure, and the rest of the figure is symmetric above and below the median. On the other hand, many of these variables do not have univariate outliers. This could be positive since they won’t interfere with the models we train. However, there are also other boxplot that show some univariate outliers.

We will explore how many outliers are present in each variable.
...
On the other hand, in all the variables involving the temporal dimension (those that have 'Year' in their name), there are outliers, what raise suspicions that these outliers might be related to each other. We will display a correlation matrix of the quantitative variables to understand the relationships between the variables.
...
All the variables that represent a temporal dimension are highly related to each other, which strengthens the previous idea that their outliers could be related. Additionally, there are other variables that correlate with the temporal ones, specifically age and monthly salary. The rest of the variables do not show any type of relationship. It is curious to note that the variable 'TrainingTimesLastYear' is not correlated with any variable, and it was the variable with the highest number of outliers.

### 2.3.5 - Multivariate outliers and their relation with univariates.

We will now perform a multivariate analysis of the outliers in the variables that we have observed to be related. If there exist multivariate outliers, we will also verify if are at the same time univariate outliers. This will help us to better understand the composition of those.
...
For studying multivariate outliers, we will use the mahalanobis_distance function from the rstatix package to mark all values that represent outliers according to this distance.
...
In some cases the number of univariate outliers is higher than the number of multivariate, and in others is smaller. Now we will verify what happens if we only observe the cases where the observations are mulitivariate outliers.

```{r}
colSums(vars_high_cor[vars_high_cor["is.outlier"]==TRUE,])
```

In some variables, most of the multivariate outliers are at the same time univariate. However, in other cases, this is not happening (with the variable "YearsInCurrentRole", only 5 observations are univariate outliers). So we can conclude that some of the outliers of these variables are related, but not most of them.
...
Now, we will convert the remaining character variables to factor type.
...
Some of the variables we identified as ordinal are those ending in "Satisfaction". We lack information about their actual range since the dataset download link didn't provide the theoretical scale range. This means the variables might theoretically have values beyond those in this sample, making it difficult to interpret the values accurately. We can only rank the values without knowing their actual magnitude.

To enhance interpretability, we can create two binary variables based on the satisfaction scores:

**Variable 1:**  'Max_satisfaction' will be set to "1" if at least two of the satisfaction variables have the highest value in the sample, and "0" otherwise.

**Variable 2:** 'Min_satisfaction' will be set to "1" if at least two of the satisfaction variables have the lowest value in the sample, and "0" otherwise.

This approach helps to identify the most and least satisfied individuals by distinguishing them based on extreme values in the satisfaction metrics.
...
In all cases, in terms of standard deviations, the maximum distance of the highest outlier from the mean has decreased. Therefore, the transformation in these cases also has the desired effect, reducing the extremity of the outliers.
...
Previously, the dataset was prepared with both discrete and normalized versions of the quantitative variables using min-max scaling. However, the results from the model built with these normalized variables are not satisfactory. As a result, two models will be developed in parallel: one using min-max normalization and the other using standardization (mean of 0 and standard deviation of 1). This comparison will help assess the impact of different scaling methods on the model's performance.

Qualitative variables will be excluded as this model require cuantitative variables. Since we are using fewer variables than available in the dataset, we will just explore if the observations naturally group into distinct clusters or if further refinement in variable selection is needed to achieve clearer groupings.
...
There are many branches in both cases, but their shapes change depending on the scale of the variables. We didn't set the number of clusters, as this depends on where we cut the tree. This will determine the number of branches. Each cut branch will be a different cluster.

In the next step, we will determine the number of clusters that yield the best result.

## 5.2 - Analysis of the results of the model.

We will create a function that returns a vector with the medium silhouette of the model, with different number of clusters (the function will test multiple number of clusters in each model and get the medium silhouette for each case)
...
The highest average silhouette score is obtained with 2 clusters using both scaling methods. However, when using standardized distances, the result is much better (the Y-axis values in this second case are higher).
...
Depending on the scale of the variables, the effectiveness of the model changes. Although with both scales the number of clusters with the highest average silhouette is 2, when we standardize the data, the average silhouette is higher than when we normalize it. For this reason, the best model is the 2-cluster model obtained with standardized data. However, the maximum average silhouette obtained is 0.3. Thus, the model does not seem to be producing very solid clusters.

It is also important to consider that since we are only using quantitative variables (as distance-based models cannot use categorical variables), we might be missing information that could be very important for differentiating groups in the data. However, this is a characteristic of the model; since it operates based on distances, there is nothing we can do to address this problem.
...
With a significant increase in the neighborhood criterion (minPts), the value of "eps" has increased only slightly. In the first model, it is 7.327, and in the second model, it is 8.8934. This means that to find 4 times more points, the search radius only needs to increase slightly.
...
The difference between both results is not very big. The unique difference is that in the first model, the distance for each observation increases in a more progressive way than in the second model. However, the distances on both models are big.
...
The cluster in the first model has grown even more and now occupies almost all the observations. In the second model, a cluster of nearly the same size as the cluster in the first model has abruptly appeared.

The results reveal interesting patterns. With a relatively low minPts value (100), a cluster gradually forms as the eps_cl parameter increases. On the other hand, when minPts is increased to 400, only noise is initially identified, reflecting significant rigidity in neighborhood definition due to the high density required to form a cluster. However, when eps_cl is raised to 3.5, a clustering pattern similar to that of the model with a lower minPts value emerges. This suggests that the data possess an underlying dense structure that only becomes evident under broader distance thresholds.

## 6.3 - Getting a measure of the quality of the clustering.

First, we will examine the appearance of these 2 clusters representing them with 2 principal components. We will to this visualization with the 2 last models.
...
In both models there is only a single cluster of points, with some points gradually moving away from this structure. Both models capture a large portion of the points with the highest density and exclude most of the points with lower density. Therefore, it seems logical that the model does not find more than a single cluster.

Since we have only one cluster, we will assess its quality using the "Sum of Squares Within" index. This evaluation will be done for the model with an optics value of 400 before applying DBSCAN. We will calculate and compare the index and its average value for points inside the cluster versus those outside to determine the density of the cluster.
...
print(paste0("Sum of Squares Within inside the cluster: ", average_ssw_cluster))
print(paste0("Sum of Squares Within outside the cluster: ", average_ssw_non_cluster))
```

The mean value of the sum of squared distances to the centroid differs significantly between the points inside the cluster and those outside it. Therefore, it is clear that the cluster identified by the model is dense. However, is not clear if with this data using a method like this is more useful than applying an outlier detection method.
...
This project is a migration of other project. When initially I did this code, I forgot to delete the variable "Over18" until this point of the project. If I delete before in the script, some parts stop working because refactoring is needed. For now, this variable will be eliminated in this point of the project. The most adequate solution will be to delete it in the part of the script where all de variable elimination are done and refactor the code. But for now, we keep this solution, because is not a serious problem.
...
Before starting with train test split, we will select only the variables most appropiate variables for building the model. We will consider 2 things; first, we will not select more than one version of the the same variable, and second, we will choose only the most important/relevant ones for building our model.
...
We used a proportion of 1/5 for splitting the dataset, because the size of the dataset is not too small (in that case, using a similar length for both datasets would be better), but the size is not too big (if it had hundreds of thousands of observations, we could have used a smaller proportion for the split).
...
The tree is quite complex, with multiple branches, and it is quite deep. The nodes and texts within it are generally legible, but the complexity of the tree gives it an appearance that is overloaded with information.

Now, for a fast evaluation of this model (that is only an initial model) we will create a confusion matrix.
...
The accuracy of the model is relatively high, however, the model doesn't predict very well the class "1" (the employee left the organisation). For this class we had a lot of very specific rules, but, those rules probably were very specific to the train dataset, and is not applying to other cases.

We will try to solve this problem using pruning options, this will avoid creating overly complex ramifications, and will lead to more general rules that will likely be useful in various datasets.
...
Now the model is much more understandable than before. If this model is also better (or equal) for predicting both classes, we will be able to say that pruning it was something positive.
...
calculateMetrics <- function(TP, TN, FP, FN) {
...
If we compare this confusion matrix with the previous one, the results of this model are slightly worse, because this model makes some more false positives -22 vs 19 that were before-, but considering that is a much more simple model, we will not consider this as an important problem.

Now we use the function.

```{r}
calculateMetrics(15, 226, 22, 31)
```
...
The value of the **specificity** (0.91) is higher than the accuracy, which indicates that the model is better for predicting the negative class than the positive. On the other hand, the low values of the **precision** (0.41) and the **sensitivity** (0.33) are indicating that the model is not very good at detecting the positive class.

This interpretation is confirmed by the value of the **F-Measure** (0.36), which is low. This indicates that the model has not a good balance between the number of cases correctly detected of each class (having a worse capacity at detecting the negative class).
...
Given that this model has a low value in the F-Measure, is possible to improve it, and having better general results for predicting both classes (and even, if the precision were much higher, it would be possible to use this model for more specific situations and not only for general descriptions of the situation of an organisation).
...
# 8 - LOGISTIC REGRESION MODEL

We attempted to use mutiple relatively complex models, and the results were not very satisfactory (the decision tree model is not effective at identifying the positive class, the hierarchical clustering has a very low average silhouette even in the best obtained result, and the DBSCAN algorithm finds only one cluster, which makes unclear if this method is much better than just using a multivariate outlier detection method). 

Therefore, we will use a simpler model, the logistic regression. This will serve as a benchmark. If other models are trained, they should at least work better than this one to be considered an improvement.

**Note.**

The best way to train this model is to verify all the statistical assumptions of the model, and only use the model if they are met. However, since we are using cross validation to evaluate its performance, we will accept not verifying them if the model performs well with other data than the training data. However, in a real scenario, where critical problems are analysed, it is not a good practice to use the model without meeting the statistical assumptions.

## 8.1 - Creating the model

```{r, warning=FALSE}
set.seed(7)
# We create a lineal model with the "glm" function from the binomial family, and using all the variables from the training dataset
logictic_model <- glm(Attrition ~ ., data = train_dat, family = binomial)
# We show a summary of the model
summary(logictic_model)
```

This summary shows us which variables are statistically significant in the prediction, and the contribution of each variable (in the "Estimate" column). The values from this summary are used to predict the probability of each observation belonging to a particular class. Now we will make predictions with the model.

## 8.2 - ROC curve of the model

```{r}
predictions <- predict(logictic_model, newdata = test_dat, type = "response")
head(predictions)
```

These predictions indicate the probability that the model assigns to each observation of being positive. To determine the class of each observation, we will establish a cut-off point for the predictions. If an observation has a probability equal to or higher than this point, it will be classified as positive (class 1), but if it has a lower value, it will be classified as negative.

To decide the value of this point, we will visualize the ROC curve of this model.


```{r, warning=FALSE}
# We will use the "ROCR" library for evaluate the model
library(ROCR)
# We will create a "prediction" object with which we will use the "performance" function for evaluating it
prediction <- prediction(predictions, test_dat$Attrition)
prf <- performance(prediction, measure = "tpr", x.measure = "fpr")
# We graphically show the performance of the model
plot(prf)
```

This ROC curve is moved away from the diagonal, to the upper left corner. This is something positive because it means that while the cut-off point increases, the false positives increment is slower than the true positives increment. For example, in this graph we can see a zone where the rate of false positives is still quite low (around 0.4), but the true positive rate is much more higher (around 0.8).

We will try to create a model which obtains these results. For this, we will transform the values of the probabilities in 0 and 1 using different cut-off points, and establishing confusion matrices with the obtained values and the real values of the "Attrition" variable.


## 8.3 - Searching a balanced model

**We start trying a cut-off point of 0.5**

```{r}
prediction_data <- ifelse(predictions > 0.5, 1, 0)
# We create a confusion matrix with the obtained predictions
table(test_dat$Attrition, prediction_data)
```

This model performs better than the previous decision tree in predicting class 0, correctly identifying 240 cases compared to 226 that predicted the previous decision tree, while predicting a similar number of class 1 cases (14 vs. 15). It also reduces false positives from 22 to 8, making it a slightly better model overall. However, we will test other cut-off points to improve predictions for the positive class by lowering the threshold for classifying an observation as positive.

**We try a cut-off point of 0.25**

```{r}
prediction_data <- ifelse(predictions > 0.25, 1, 0)
# We create a confusion matrix with the obtained predictions
table(test_dat$Attrition, prediction_data)
```

The number of cases predicted as positive has increased, and the number of cases predicted as negative has decreased. However, the number of correct cases predicted as class 0 has not decreased a lot, but the number of cases correctly classified as class 1 is twice as many as with the previous cut-off point value. Nevertheless, the number of false positives has increased by a higher proportion.

Even so, depending on the use given to the model, this may not necessarily be negative. If it is more important to maximize the number of positive cases detected than minimizing the number of cases incorrectly classified as positive, then this model can be useful in this case.

**We try a cut-off point of 0.2**

```{r}
prediction_data <- ifelse(predictions > 0.20, 1, 0)
# We create a confusion matrix with the obtained predictions
table(test_dat$Attrition, prediction_data)
```

In this case, the increase of true positives is insignificant, with only 3 more cases than with the previous model, compared with the decrease in true negatives detected, which is 27 fewer cases correctly identified. We can not say that this model is better than the previous one.

## 8.4 - Evaluating the selected model

We will calculate the metrics of the second confusion matrix that we have obtained, the one with a cut-off point of 0.25.

```{r}
# First, we will save as variables each type of correct and incorrect predictions
...
# We use the previously created function for calculating the confusion matrix metrics
calculateMetrics(TP, TN, FP, FN)
```

We can see that the metrics of this model are better those obtained with the last decision tree. The only metric that has decreased, is specificity (it has decreased from 0.91 to 0.86). All the other metrics are higher, and some of them have significant increases, such as, the sensitivity (it has increased from 0.33 to 0.67), and the F-Measure (has increased from 0.36 to 0.57). We can attribute this increase to the fact that the number of true negatives has not decreased hugely, but the number of true positives has doubled. This indicates that the balance between true positives and true negatives has improved in this model.

# 9 - CONCLUSIONS AND POSSIBLE IMPROVEMENTS TO THE PROJECT

First of all, the **exploratory data analysis** in this project was very general, since we focused on understanding the nature of all variables, and their general characteristics and interrelations. This is something useful for understanding the generalities of the problem, and can be useful as a first approach. However, doing a more specific analysis focusing on the target variable can also be useful. If a better model is to be developed (with a higher capacity of detecting the class 1 and not only the class 0), it would be necessary to analyse the characteristics of the variables considering both classes of the target variable, in order to better guide the creation of the model.

Related to the **Singular Value Decomposition technique**, since this is useful for understanding the behavior of the data in terms of the variance explained by the variables, a deeper exploration should be conducted, in order to understand which variables are the most important in this regard. Complementary techniques could also be used to explore the dimensionality reduction from multiple perspectives. The techniques include, TSNE or UMAP, as those permit finding possible clusters in a low dimensional space. Additionally, PCA would be a good option, as this is a widely accepted and used technique for this purpose.

On the other hand, a similar issue to what occurred in the EDA arises in the sections where **clustering techniques** are applied. The approach was very generic, as we utilized the techniques on all quantitative variables, which only permits the identification of clusters if they are highly apparent in the data. In this case, the clusters are not readily apparent and may only be discovered through a more focused analysis of specific target variables that could potentially form clusters. It would be beneficial to use clustering techniques with pre-defined hypotheses about the types of clusters expected, selecting only the relevant variables based on these hypotheses. This approach could improve the identification of distinct clusters. Nevertheless, the techniques employed in this project helped us assess the clarity of the data structures, revealing that the structures were not very clear with the methods used.

Regarding the **unsupervised learning techniques** used (decision trees and logistic regression), although we identified a relatively acceptable model—particularly the logistic regression with the last cut-off point value—there is significant room for improvement. Firstly, there are multiple unsupervised learning techniques with various parameters that can be manipulated. Testing different combinations of these parameters during model training could yield better results. A more in-depth exploration of models and parameter combinations using a grid search strategy should be conducted to find the optimal model for this use case. Examples of models not explored in this project include support vector machines, random forests, neural networks, and ensemble models combining multiple techniques.