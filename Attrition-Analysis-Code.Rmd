...
This model works better for predicting the 0 class than the last obtained decision tree, (is predicting correctly 240 cases compared with the 226 that predicted the previous model). On the other hand is predicting practically the same number of class 1 cases (14 cases compared with the 15 predicted by the last obtained decision tree), at the time that the number of false positives has decreased (in the previous model were 22, and in this one are 8). Therefore we obtained a little bit better model in this case.

Anyway, we will test the predictions of the model with other values for the cut-off point. As this model is very good predicting the negative cases, but not the positive, we will decrees the cut-off point value, and consequently establish a lower requirement for consiering an observation as a positive case.
